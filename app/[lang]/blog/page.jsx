      ...
      <div className="flex-grow pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-16">
            ...
            <p className="text-xl text-gray-400 max-w-2xl">{copy?.desc}</p>
          </div>

          {posts.length === 0 ? (
            <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/10">
              <h2 className="text-2xl text-white font-semibold">{copy?.emptyTitle}</h2>
              <p className="text-gray-400 mt-2">{copy?.emptyDesc}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <Link
                  ...
                  className="group relative bg-white/5 rounded-3xl overflow-hidden border border-white/10 hover:border-primary/50 transition-colors"
                >
                  {post.mainImage && (
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-500 border-t border-white/10 pt-6">
                      <span>{post.authorName || copy?.authorFallback}</span>
                      <span>
